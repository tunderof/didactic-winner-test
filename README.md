...
Для сборки и запуска основного приложения нужны:
...
## Требования к Python-генератору

Для `generate_test_file.py` нужен Python 3.9 или новее

На Windows удобнее запускать через Python Launcher:

```powershell
py generate_test_file.py -s 100MB -t bin
```

Если команда `py` недоступна, можно использовать обычный запуск через `python`
...
```powershell
py -m unittest tests\test_generate_test_file.py
```
...
Скрипт `generate_test_file.py` создает случайный `.bin` или `.txt` файл в текущей папке запуска
